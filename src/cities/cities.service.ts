import { ForbiddenResult, NotFoundResult } from '../../shared/errors';
import { City, GetCityResult } from './cities.interfaces';
import { CitiesRepository } from './cities.repository';

export class CitiesService {
  public constructor(private readonly _repo: CitiesRepository, private readonly _env: NodeJS.ProcessEnv) {
  }

  public getCity(id: number): Promise<GetCityResult> {
    return new Promise(async(resolve: (result: GetCityResult) => void, reject: (reason: NotFoundResult) => void): void => {
      if (!this._repo.exists(id)) {
          reject(new NotFoundResult('UNKNOWN_CITY', 'There is no city with the specified ID!'));
          return;
      }

     
      const city: City = await this._repo.getCity(id);
      const result: GetCityResult = {
        city
      };

      resolve(result);
    });
  }

  public getAllCities(): Promise<GetCityResult> {
    return new Promise(async (resolve: (result: GetCityResult) => void, reject: (reason: NotFoundResult) => void): void => {     
      const city: any = await this._repo.getAllCities();
      const result: GetCityResult = {
        city
      };
      console.log(result);
      resolve(result);
    });

  }


  public createCity(data:Object): Promise<GetCityResult> {
    return new Promise(async (resolve: (result: GetCityResult) => void, reject: (reason: NotFoundResult) => void): void => {     
      const city: any = await this._repo.createCity(data);
      const result: GetCityResult = {
        city
      };
      console.log(result);
      resolve(result);
    });

  }


}
